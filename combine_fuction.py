...
def step_2d(
    ...
    """
        ...
        angular_threshold [float] : Angular scale of the step threshold
    ...
    ring = (rr <= (image_scale*pixscale/angular_threshold))
    iring = 1-ring
    
    kfft = np.fft.fftshift(ring)
    ikfft = 1-kfft
...
def plot_ps(
    ...
    for i in range(len(angular_scales)):
        ...
        plt.axvline(x= 1. / (angular_scale / pixscale) ,
...
def feather_2d(
    ...
    kfft = np.abs(np.fft.fft2(kernel))
